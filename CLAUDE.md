...
3. **API: czysta ekstrakcja** — proxy do Tiki: upload pliku, walidacja typu/rozmiaru,
   obsługa błędów (Tika niedostępna, plik nieobsługiwany, pusty wynik OCR). Osobny
   problem: śmieciowa warstwa tekstowa (PDF z PUA) wymaga detekcji + OCR-fallbacku —
   „pusty wynik" jako jedyny warunek błędu nie wystarcza; pełny opis i zmierzone dowody
   w „Plan kroku 2.3 → 2.3.5".
...
- **Fallback ekstrakcji dla śmieciowej warstwy tekstowej (PDF z PUA)** — DECYZJA DO
  PODJĘCIA; pełny opis, zmierzone dowody i otwarte pytania (jak wykrywać PUA, fallback
  per-request vs globalny, jak ciąć do N stron, co asertować) w „Plan kroku 2.3 → 2.3.5".
  Test integracyjny PDF jest WSTRZYMANY do tej decyzji; test OCR dla `samples/sample_01.jpg`
  jest niezależny i może powstać wcześniej.
...
**Otwarte decyzje (DO PODJĘCIA na starcie 2.3.5, patrz „Kontrakty"):** (1) jak wykrywać PUA
(próg udziału znaków PUA / brak liter alfabetu / heurystyka długości); (2) fallback jako
per-request `X-Tika-PDFOcrStrategy: OCR_ONLY` czy globalna zmiana `ocrStrategy` w
`tika-config.xml` (globalnie `OCR_AND_TEXT`/`OCR_ONLY` spowalnia wszystkie PDF-y); (3) jak
technicznie odciąć do N stron (własny podział pliku przed Tiką vs natywny limit
`tika-server` — sprawdzić wobec upstreamu, nie zgadywać); (4) co asertować na
`samples/sample_01.pdf`. Test PDF **WSTRZYMANY** do tej decyzji jest tu odwieszany. Config:
`MAX_OCR_PAGES` w `Settings` + `.env.example`. Testy: jednostkowe detekcji PUA (atrapa
transportu) + integracyjny PDF przez endpoint (treść poprawna po OCR-fallbacku).
